...
public class ExcelComparatorUtils {
    ...
    private static final String ROW_IS_EMPTY = "Row is empty ::";
    ...
    private static String getFilePath() {
        return Paths.get("").toAbsolutePath() + File.separator + "src" + File.separator + "main" + File.separator + "resources" + File.separator;
    }


    public static void main(String[] args) throws Exception {
        String SAMPLE_XLSX_FILE_PATH = getFilePath() + "provawrite1.xlsx";
        String SAMPLE_XLSX_FILE_PATH1 = getFilePath() + "provawrite11.xlsx";

        try (Workbook wb1 = WorkbookFactory.create(new File(SAMPLE_XLSX_FILE_PATH), null, true)) {
            try (Workbook wb2 = WorkbookFactory.create(new File(SAMPLE_XLSX_FILE_PATH1), null, true)) {
                for (String d : ExcelComparatorUtils.compare(wb1, wb2)) {
    ...
    public static List<String> compare(Workbook wb1, Workbook wb2) {
        ...
        ExcelComparatorUtils excelComparatorUtils = new ExcelComparatorUtils();
        excelComparatorUtils.compareNumberOfSheets(loc1, loc2);
        excelComparatorUtils.compareSheetNames(loc1, loc2);
        excelComparatorUtils.compareSheetData(loc1, loc2);

        return excelComparatorUtils.listOfDifferences;
    }
    ...
    private void compareDataInAllSheets(Locator loc1, Locator loc2) {
        int sheetNum = Math.min(loc1.workbook.getNumberOfSheets(), loc2.workbook.getNumberOfSheets());
...
//            }
    ...
    private void compareDataInSheet(Locator loc1, Locator loc2) {
        int lastRowNum = Math.min(loc1.sheet.getLastRowNum(), loc2.sheet.getLastRowNum());
...
//            }
            ...
            if (loc1.row == null && loc2.row != null) {
                addMessageOfRow(loc1, loc2, ROW_IS_EMPTY, null, loc2.row.getRowNum());
                continue;
            }

            if (loc2.row == null && loc1.row != null) {
                addMessageOfRow(loc1, loc2, ROW_IS_EMPTY, loc1.row.getRowNum(), null);
                continue;
            }
    ...
    private void compareDataInRow(Locator loc1, Locator loc2) {
        int lastCellNum = Math.min(loc1.row.getLastCellNum(), loc2.row.getLastCellNum());
...
//            }
            ...
            if (loc1.cell == null && loc2.cell != null) {
                ...
                String cellValue = dataFormatter.formatCellValue(loc2.cell);
                if (cellValue.equals("")) //ignoring if cell is empty
                    continue;
                addMessage(loc1, loc2, CELL_DATA_DOES_NOT_MATCH, null, cellValue);
                continue;
            }
            if (loc2.cell == null && loc1.cell != null) {
                DataFormatter dataFormatter = new DataFormatter();
                String cellValue = dataFormatter.formatCellValue(loc1.cell);
                if (cellValue.equals("")) //ignoring if cell is empty
                    continue;
                addMessage(loc1, loc2, CELL_DATA_DOES_NOT_MATCH, cellValue, null);
                continue;
            }
...
//            }
    ...
    private void addMessageOfRow(Locator loc1, Locator loc2, String messageStart, Integer rowNum1, Integer rowNum2) {
        String str = String.format(Locale.ROOT, "%s\nworkbook1 -> %s [%s] != workbook2 -> %s [%s]",
                messageStart,
                loc1.sheet.getSheetName(), rowNum1,
                loc2.sheet.getSheetName(), rowNum2
        );
        listOfDifferences.add(str);
    }
    ...
    private void addMessage(Locator loc1, Locator loc2, String messageStart, String value1, String value2) {
        ...
        String str =
                String.format(Locale.ROOT, "%s\nworkbook1 -> %s -> %s [%s] != workbook2 -> %s -> %s [%s]",
                        messageStart,
                        loc1.sheet.getSheetName(), dataFormatter.formatCellValue(loc1.cell), value1,
                        loc2.sheet.getSheetName(), dataFormatter.formatCellValue(loc2.cell), value2
                );
        listOfDifferences.add(str);
    }
